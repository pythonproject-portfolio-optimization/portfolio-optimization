...
def optimize_portfolio(stocks):
    ...
    for i in range(num_portfolios):
        ...
        results[0,i] = round(portfolio_return,3)
        results[1,i] = round(portfolio_std_dev,3)
        
        #store Sharpe Ratio (return / volatility) - risk free rate element excluded for simplicity
        results[2,i] = round(results[0,i] / results[1,i],3)
        
        #iterate through the weight vector and add data to results array
        for j in range(len(weights)):
            results[j+3,i] = round(weights[j],3)
    ...
    minmax_list = [max_sharpe_port, min_vol_port]
